...
function SignAvatar({ isPlaying }: { isPlaying: boolean }) {
  return (
    ...
    >
      ...
      {/* ── ARMS & HANDS ── */}
      {/* These animate when isPlaying */}

      {/* LEFT ARM */}
      <g
        style={{
          transformOrigin: '75px 125px',
          animation: isPlaying ? 'leftArm 1.6s ease-in-out infinite alternate' : 'none',
        }}
      >
        <rect x="55" y="115" width="22" height="12" rx="6" fill="#4f46e5" />
        {/* Left Hand */}
        <g style={{
          transformOrigin: '55px 122px',
          animation: isPlaying ? 'leftHand 1.6s ease-in-out infinite alternate' : 'none',
        }}>
      ...
      <g
        style={{
          ...
          animation: isPlaying ? 'rightArm 1.6s ease-in-out infinite alternate-reverse' : 'none',
        }}
      >
        <rect x="123" y="115" width="22" height="12" rx="6" fill="#4f46e5" />
        {/* Right Hand */}
        <g style={{
          transformOrigin: '145px 122px',
          animation: isPlaying ? 'rightHand 1.6s ease-in-out infinite alternate-reverse' : 'none',
        }}>
...
export default function SignLanguageAvatar() {
  ...
  const handleStop = () => {
    ...
    setCurrentSentenceIndex(0);
    setProgress(0);
    setCurrentText("Çeviri durduruldu. Tekrar başlatmak için ▶ butonuna basın.");
  };
      ...
      {isOpen && (
        ...
        >
          ...
          <div className="flex items-center justify-between px-4 py-3"
            style={{ background: 'linear-gradient(90deg,#4338ca,#6d28d9)' }}>
          ...
          <div className="relative" style={{ aspectRatio: '4/3', background: 'linear-gradient(180deg,#1e1b4b,#0f0e1a)' }}>
            {hasError ? (
              <div className="flex flex-col items-center justify-center h-full gap-2 text-red-400 p-4 text-center">
                ...
                <p className="text-xs opacity-70">Yapay zeka servisine ulaşılamıyor.</p>
              </div>
            ) : (
          ...
          <div className="px-4 py-3 border-t" style={{ borderColor: 'rgba(255,255,255,0.08)', minHeight: '64px' }}>
            <div className="flex items-start gap-2">
              ...
              <p className="text-xs leading-relaxed italic" style={{ color: '#c7d2fe' }}>
                {currentText}
              </p>
            </div>
          </div>
